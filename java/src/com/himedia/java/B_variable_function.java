...
- 정수형 : byte,short,int,long
    정수를 저장하는게 사용된다. byte는 이진데이터를 다룰 때, short는 C언어와의 호환을 위해 추가되었다.
    크기 : byte(1byte-8bit), short(2byte), int(4byte), long(8byte)
    범위 : byte(-128 ~ 127), short(-32,768 ~ 32,767), int(-2,147,483,648 ~ 2,147,483,647),
...
2. 함수 종류 : return 문이 실행되면, 해당 함수는 즉시 종료가 된다.
...
public class B_variable_function {
    ...
    public static void exam1() {
        ...
        fNum = (float) 3.146546543263545361654;
        System.out.println(fNum);

        final float PI = 3.14f; // final:변경할 수 없다.
        // PI = 3.15; // Error

    }

    /*
     * 함수 : 프로그래밍에서 특정 잡업을 수행하기 위해 작성된 코드의 묶음*
     * 1. 함수 선언 : 함수의 이름과 특성을 정의하는 부분이다.
     * 2. 매개변수(Parameters) : 매개변수는 함수가 작업을 수행하는데 필요한 입력값을 전달받는 부분이다.
     * 3. 반환 타임(Return type) : 함수가 어떤 유형의 값을 반황할지를 정의.
     */
    ...
    public static void main(String[] args) {
        ...
        double dresult3 = divide(a, b);
        System.out.println(dresult3);
    }
}