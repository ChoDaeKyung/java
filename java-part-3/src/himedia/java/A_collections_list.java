...
// - 중복 허용 : 동일한 값을 가진 요소가 여러개 있을 수 있다.
...
// * 요약
// - ArrayList는 배열 기반의 리스트로, 인덱스를 통한 빠른 접근이 가능하지만 중간 삽입/삭제가 느리다.
// - LinkedList는 노드 기반의 리스트로, 삽입/삭제가 빠르지만 인덱스를 통한 접근이 느리다.
// - Stack은 후입선출(Last In First Out(LIFO))를 가지며, 요소를 스택에 추가하고 제거하는데 사용된다.
...
//        System.out.println("List after clear : " + list);
    ...
    // 3. Stack
    // LIFO 구조 : Stack은 Last-In_Last-Out(후입선출) 구조를 따른다.
    // - Vector 기반 : Stack은 Vactor 클래스 상속받아 구현한다.
    // - 주요 메서드 : push() 요소삽입, pop() 요소제거, peek() 맨 위의 요소를 확인, empty() 스택이 비어있는지 확인
